...
| 라우트 | 목적 | 데이터 | 내비게이션 |
|---|---|---|---|
| `/` | 활성 플랜 편집·라이브 요약·발권 진입 | Dexie | `플랜`, `검색` 2개 탭 중 플랜 |
| `/search` | 검색·직접 추가 | BFF search + Dexie | 2개 탭 중 검색, 뒤로가기는 브라우저 이력 존중 |
| `/ticket` | 현재 플랜 티켓 미리보기·PNG·링크 공유 | Dexie + share API | TabBar 없음, 뒤로 플랜 |
...
P0에는 `/list/[id]`, `/discover`, `/history`, `/auth`를 만들지 않는다.
