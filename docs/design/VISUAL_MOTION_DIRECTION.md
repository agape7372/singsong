...
현재 적용 심볼은 **Folded Session S**다. 넓은 ticket strip이 두 번 접혀 하나의 굵은 S 흐름을 만들며, cream canvas 위에 rose 상·하단 fold와 black diagonal middle span을 둔다. 상·하단에는 각각 한 개의 cream registration hole만 사용한다. production source of truth는 [`public/icons/icon.svg`](../../public/icons/icon.svg)이며 생성 탐색 PNG는 기하 참고 기록이다.

요구사항:

- 현재 32px 헤더와 180/192/512px 설치 아이콘에서 실루엣 식별 가능
- 단색·24px 파생본은 출시 전 별도 검수하며 현재 컬러 master를 자동 축약하지 않음
...
> **⚠ 갱신 (D-027, 사용자 canonical 승인, 2026-07-23)**: 하단 PrimaryNav는 2항목이 아니라
> **4항목(플랜·보관함·발견·설정)**이다. 검색은 별도 탭/라우트가 아니라 플랜 내 bottom sheet다.
> 각 탭은 CUTLINE ledger/paper 문법을 유지하고 계획 단계에는 티켓 물성(절취선·펀칭·시리얼)을
> 쓰지 않는다(티켓 물성은 `/ticket`·`/s/[slug]`에만). 상세: `CONFLICT_REGISTER` C-30~C-34.

- 900px 미만: `/`·`/library`·`/discover`·`/settings`에서 하단 PrimaryNav 4항목(플랜·보관함·발견·설정).
- 900px 이상: 같은 nav DOM을 header 영역에 배치한다. 모바일/데스크톱 nav를 중복 렌더하지 않는다.
  4항목+wordmark 폭 확보를 위해 좁은 데스크톱(≤1099px)에서 헤더 자동저장 status를 숨긴다.
...
#### E. 티켓 플립 (앞↔뒷) — 사용자 지시 예외

> **§2-5 "no 3D"의 의도적 예외**(사용자 명시 지시, 2026-07-24). 스큐어모픽 렌더가 아니라
> 탭 토글 한정의 카드 뒤집기다.

- 발권 티켓을 탭하면 앞면(모던 리소 요약)↔뒷면(전체 곡 목록·계산 상세)이 **CSS 3D `rotateY` 플립**.
- duration `--motion-flip`(460ms) · `--ease-standard`. transform만. `backface-visibility:hidden`.
- 접근성: `role="button"`+`aria-pressed`+Enter/Space, `aria-live` 상태 알림. 공유/아카이브 액션은 플립 밖.
- reduced-motion: 회전 없이 face `opacity` 스왑.
- 적용: 보관함 콜렉션(중앙 카드)·`/ticket`·`/s`. PNG 수출은 항상 **앞면** 캡처. 구현 `flippable-ticket.tsx`.
...
- 생성 결과를 로고, PWA icon, 접근성 핵심 아이콘으로 그대로 납품하지 않는다. Folded Session S도 탐색 PNG를 참고해 제어된 SVG master로 재작도했다.
...
### 10-3. Production brand asset 목록

| 상태       | 경로                                    | 형식        | 비고                    |
| ---------- | --------------------------------------- | ----------- | ----------------------- |
| 정본       | `public/icons/icon.svg`                 | SVG         | Folded Session S master |
| 사용       | `public/icons/folded-session-s-192.png` | PNG 192×192 | manifest·metadata·헤더  |
| 사용       | `public/icons/folded-session-s-512.png` | PNG 512×512 | manifest·metadata       |
| 사용       | `public/icons/folded-session-s-180.png` | PNG 180×180 | Apple touch icon        |
| 호환 alias | `public/icons/icon-192.png`             | PNG 192×192 | 기존 설치 클라이언트용  |
| 호환 alias | `public/icons/icon-512.png`             | PNG 512×512 | 기존 설치 클라이언트용  |
| 호환 alias | `public/icons/apple-touch-icon.png`     | PNG 180×180 | 기존 Apple metadata용   |

UI 기능 아이콘은 컴포넌트 안의 직접 검수한 inline SVG를 사용한다. 존재하지 않는 예정 경로를 production asset으로 간주하지 않는다.
