...
- phone preview: `PREVIEW_RESTART_REQUIRED` — 리뉴얼 build 뒤 기존 server manifest와 새 asset이 불일치하며 외부 재공개 승인 전 재시작하지 않음
...
- compact AppHeader, mobile bottom↔wide header를 공유하는 플랜·검색 2-item nav, continuous Working Strip와 single perforation, ActionDock/PlanRail BottomSlot
...
- exact Node `24.18.0`/pnpm `11.9.0`, latest renewal format/lint/type와 39 files/190 tests PASS
...
- latest renewal current는 demo production build와 responsive Chromium 3/3 PASS; 리뉴얼 전 byte-exact 233-path clean snapshot은 37 files/185 tests, coverage threshold, PWA artifact/smoke PASS
...
- owner process: fixture app PID 29532 (`127.0.0.1:3000`), Quick Tunnel PID 32848
- lifecycle: process는 보존됐지만 최신 앱을 외부에 다시 공개하는 재시작은 사용자 승인 대기; stable host, staging 또는 production 아님
- historical public flow: 리뉴얼 전 public hostname build에서 search 200, share create 201/read 200과 Pixel 7 browser profile organizer→recipient→import 흐름 PASS
- 현재 한계: 리뉴얼 build 뒤 기존 server manifest가 정적 asset 5개에 500을 반환한다. 재게시와 asset 검증 전 preview URL을 사용 가능한 앱으로 안내하지 않음
...
- 리뉴얼 전 `.nvmrc`의 exact Node 24.18.0에서 233-path clean snapshot을 재현했다. clean은 frozen install 506 packages reused, format/lint/type, 37/185 coverage, build/PWA/smoke를 통과했다. 최신 리뉴얼 current는 별도로 39/190·build·responsive Chromium 3/3을 통과했다.
...
- retained pre-renewal coverage: `coverage/coverage-summary.json` — 37/185와 global threshold PASS; 최신 39/190은 coverage 재측정 없음
...
29532와 tunnel PID 32848은 현재 의도적으로 유지한다. clean shutdown으로 표시하지 않으며,
