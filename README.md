...
Node `24.18.0`/pnpm `11.9.0`과 Next.js `16.2.11` 기준으로 최신 CUTLINE 리뉴얼 트리는 format/lint/type, 39파일 190테스트, demo production build와 responsive Chromium 3/3을 통과했다. 리뉴얼 전 byte-exact 233-path clean snapshot은 37파일 185테스트와 coverage threshold, build/PWA/smoke를 통과했으며 보존된 이전 Chromium 결과는 20 discovered 중 13 pass와 7 intentional project-gated skip, built-PWA 3/3이다. 두 시점의 증거를 섞어 과장하지 않으며 정확한 수치·환경 재시도·남은 gate는 [VERIFICATION_REPORT.md](./VERIFICATION_REPORT.md)를 따른다.
...
`https://bond-athletics-calculations-putting.trycloudflare.com`의 app PID 29532와 tunnel PID 32848은 보존되어 있지만, 리뉴얼 build 뒤 기존 server manifest와 새 `.next`가 달라져 현재 정적 asset 5개가 500을 반환한다. 최신 앱을 외부에 다시 공개하는 재시작은 명시적 승인이 필요한 상태이므로 현재 판정은 `PREVIEW_RESTART_REQUIRED`다. 아래 설치 안내는 재게시와 asset 검증이 완료된 뒤에만 사용한다.
